...
function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    if (pathname) {
      setIsLoading(true);
      setProgress(20); // Start progress immediately

      const timer = setTimeout(() => {
        setProgress(90);
      }, 30); // Short delay to show some progress
      const finishTimer = setTimeout(() => {
        setProgress(100);
        const hideTimer = setTimeout(() => {
          setIsLoading(false);
          setProgress(0);
        }, 150); // Slightly longer to hide after reaching 100%
        return () => clearTimeout(hideTimer);
      }, 150); // Slightly shorter total time
  ...
  return (
    <div className="flex h-screen"> {/* Outer container is now a flex row, full height */}
      <Sidebar>
        <SidebarHeader>
