           ...
           {placeholderProjects.length > 0 ? (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-24 text-xs text-center">{t('maps_elements.project_table_header_id', 'ID')}</TableHead>
                    <TableHead className="text-xs text-center">{t('maps_elements.project_table_header_name', 'Project Name')}</TableHead>
                    <TableHead className="text-xs text-center">{t('maps_elements.project_table_header_pop', 'PoP')}</TableHead>
                    <TableHead className="text-xs text-center">{t('maps_elements.project_table_header_status', 'Status')}</TableHead>
                    <TableHead className="text-right w-28 text-xs text-center">{t('maps_elements.project_table_header_actions', 'Actions')}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {placeholderProjects.map((project) => (
                    <TableRow key={project.id}>
                      <TableCell className="font-mono text-muted-foreground text-xs text-center">{project.id}</TableCell>
                      <TableCell className="font-medium text-xs text-center">{project.projectName}</TableCell>
                      <TableCell className="text-xs text-muted-foreground text-center">{project.popName}</TableCell>
