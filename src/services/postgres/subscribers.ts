...
function mapRowToSubscriber(row: any): Subscriber {
  return {
    ...
    services: [], // Relations would be fetched separately
    billing: {
      balance: 0,
      nextBillDate: '',
      pastInvoices: [],
...
export async function addSubscriber(subscriberData: SubscriberData): Promise<Subscriber> {
  ...
  const placeholders = Object.keys(dataToInsert).map((_, i) => `$${i + 1}`).join(', ');
